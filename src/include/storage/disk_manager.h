...
class DiskManager {
 ...
 private:
  ...
  /**
   * get extend serial index from the physical page number.
   * the index is from 0 to (the num of used extends)-1
   */
  static uint getExtIndexFromPhyPageId(page_id_t physical_page_id);

  /**
   * get bitmap page id from the extend index
   */
  static page_id_t getBitmapPhyIdFromExtIndex(uint extIndex);

  /**
   * get the offset in a bitmap from the physical id
   */
  static uint getOffsetFromPhyId(page_id_t physical_page_id);
 private:
